...
# Función matrix-free que define el producto H*x sin construir H explícitamente.
def hamiltonian_action(x, basis, label_to_index, t, U, mu, M):
    D = len(basis)
    y = np.zeros_like(x)
    for idx in range(D):
        state = basis[idx]
        # Término diagonal: interacción y potencial químico.
        diag_val = U * 0.5 * sum(n * (n - 1) for n in state) - mu * sum(state)
        y[idx] += diag_val * x[idx]
        # Término de hopping.
        for i in range(M):
            j = (i + 1) % M
            # b_i† b_j
            if state[j] > 0:
                ...
                new_state[i] += 1
                new_state[j] -= 1
                amp = -t * np.sqrt((state[i] + 1) * state[j])
                lbl_new = compute_label(tuple(new_state))
                new_index = label_to_index.get(lbl_new, None)
                if new_index is not None:
                    y[new_index] += amp * x[idx]
            # b_j† b_i
            if state[i] > 0:
                new_state = list(state)
                new_state[j] += 1
                new_state[i] -= 1
                amp = -t * np.sqrt((state[j] + 1) * state[i])
                lbl_new = compute_label(tuple(new_state))
                new_index = label_to_index.get(lbl_new, None)
                if new_index is not None:
                    y[new_index] += amp * x[idx]
...
# ---------------------------- SIMULACIÓN CON MÉTODO MATRIX-FREE ----------------------------
def simulate_t(t_val, basis, label_to_index, A_half, B_half, A_even, B_even, N, M, U, mu):
    t_val_actual = U * t_val  # t real modificado
    D = len(basis)
    
    # Definir el operador lineal sin construir la matriz completa.
    def matvec(x):
        return hamiltonian_action(x, basis, label_to_index, t_val_actual, U, mu, M)
    
    H_operator = LinearOperator((D, D), matvec=matvec, dtype=float)
    
    # Extraer el estado fundamental usando Lanczos (eigsh usa Lanczos internamente)
    eigval, eigvec = eigsh(H_operator, k=1, which='SA')
    psi_ground = eigvec[:, 0] / np.linalg.norm(eigvec[:, 0])
    
    # Cálculo de expectativas en dos sitios
    site1 = 0
    site2 = 1
    ...
    # Cálculo de la entropía de enredamiento para dos particiones
...
def main():
    ...
    # Pre-cálculos: generación de la base completa y mapeo de etiquetas
    all_states = generate_basis(M, N)
    basis = sorted(all_states, reverse=True)
    D = len(basis)
    label_to_index = {compute_label(state): idx for idx, state in enumerate(basis)}
    ...
    # Función parcial para cada valor de t/U
    ...
    # Ejecución en paralelo para distintos valores de t/U
