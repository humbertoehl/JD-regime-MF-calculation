import os
# Limitar BLAS/MKL a 1 hilo por proceso para evitar oversubscription
...
def mean_field_iteration_bond(initial_params, params, b, b_dag, b2, b2_dag, n_op, I, tol=1e-5, max_iter=150):
    # Condiciones iniciales para las 4 sublattices
    ...
    for it in range(max_iter):
        psi_old = psi.copy()
        rho_old = rho.copy()
        # Término global: suma de productos con signo alternado
        E_eta = 0.0
        for i in range(4):
            ip = (i+1) % 4
        ...
        for i in range(4):
            ip = (i+1) % 4
            im = (i-1) % 4
            # Término eta para la sublattice i
            tilde_eta[i] = (z * ((-1)**(i+1)) / 8.0) * E_eta
            t_phi[i] = t0 - g_eff * N_s * JB_abs_sq * tilde_eta[i]
            beta_i = psi[ip].conjugate() * b + psi[ip] * b_dag - \
                     (psi[i].conjugate()*psi[ip] + psi[i]*np.conjugate(psi[ip])) * I
            # Constante tilde_c para la subred i
            ...
            for s, j in [ (1, ip), (-1, im) ]:
                # Se reemplaza b^2 del vecino por psi_j^2 y n por rho_j.
                psi_j_sq = psi[j]**2
            ...
            # Hamiltoniano efectivo para la subred i
...
def compute_grid_point_bond(args):
    # Se aplica el mean-field para cada punto (mu, zt) usando la búsqueda exhaustiva de condiciones iniciales
    ...
    # Calcular diferencias de fase asegurando la envoltura en [-pi, pi]
    phase_diff = []
    for i in range(4):
        ip = (i+1) % 4
        diff = np.angle(np.exp(1j*(np.angle(psi_vals[ip]) - np.angle(psi_vals[i]))))
        phase_diff.append(diff)
    phase_diff = np.array(phase_diff)
    
    # Se omite el print dentro de la función paralela para minimizar I/O
...
def sweep_phase_diagram_parallel_bond(mu_vals, zt_vals, base_params, b, b_dag, b2, b2_dag, n_op, I,
    ...
    for res in results:
        ...
        # Se guardan todas las fases individualmente
        data_rows.append([mu_val, zt_val, psi1, psi2, psi3, psi4, rho1, rho2, rho3, rho4,
                          avg_psi, avg_phase_diff, phase_diff[0], phase_diff[1], phase_diff[2], phase_diff[3]])
    return data_rows
...
if __name__ == '__main__':
    ...
    psi_candidates = [
        ...
        [0.101+0j, 0.102+0j, 0.103*np.exp(1j*0.2), 0.104*np.exp(1j*0.2)],  
        [0.101+0j, 0.102+0j, 0.103*np.exp(1j*0.5), 0.104*np.exp(1j*0.5)],  
        [0.101+0j, 0.102+0j, 0.103*np.exp(1j*0.8), 0.104*np.exp(1j*0.8)]
    ]
    rho_candidates = [
        [0.5, 0.5, 0.5, 0.5],
        [0.5, 0.4, 0.5, 0.4],
    ...
    output_filename = "JB-Results(7)_optimized.txt"
    with open(output_filename, "w") as f:
        ...
        for row in data_rows:
            # Se guarda el valor absoluto para los parámetros complejos
            psi_vals = [f"{np.abs(row[i]):.6f}" for i in range(2,6)]
            line = "\t".join([f"{row[0]:.6f}", f"{row[1]:.6f}"] +
                             psi_vals +
                             [f"{row[i]:.6f}" for i in range(6,10)] +
                             [f"{row[10]:.6f}", f"{row[11]:.6f}",
                              f"{row[12]:.6f}", f"{row[13]:.6f}", f"{row[14]:.6f}", f"{row[15]:.6f}"])
            f.write(line + "\n")
